    ...
    try {
        if (event.action == 'add' && isWelkom) {
            ...
            const { contact, groupMetadata, name } = gChat
				await kill.sendTextWithMentions(event.chat, `Holaaaa @${event.who.replace('@c.us', '')}! 🥰 \n\nSea muy bienvenido a ${name} 😎 \n\nDeseo que te diviertas y que sigas nuestras reglas! ✅ \n\nSi es necesario, llame a un administrador o escriba /menu. 👨🏻‍💻 Dejame presentarme, soy Irís, un bot creado por saltos27, espero caerte bien, y porfavor, *NO ME SATURES*🙂`)
			}
